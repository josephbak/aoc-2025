...
int main(int argc, char* argv[]) {
   ...
   // Precompute outside the region loop (orientations don't depend on W, H)
    ...
    // if (total_cells_needed > area) {
        // std::cout << W << "x" << H << ": IMPOSSIBLE (need " << total_cells_needed << ", have " << area << ")" << std::endl;
    // } else if (total_cells_needed == area) {
        // std::cout << W << "x" << H << ": EXACT COVER" << std::endl;
    // } else {
        // std::cout << W << "x" << H << ": UNDERFILL (need " << total_cells_needed << ", have " << area << ")" << std::endl;
    // }
    ...
    //     }
        ...
        // Use PB constraints instead of ite sum
        // solver.add(z3::atleast(vec, required));
        // solver.add(z3::atmost(vec, required));
    // }
    ...
    // Before solver.check()
    // z3::params p(ctx);
    // p.set("timeout", 5000u);  // 5 second timeout per region
    // solver.set(p);
    ...
    // } else {
    //     std::cout << "Result: TIMEOUT/UNKNOWN" << std::endl;
    // }

}
    
    // std::cout << solvable_count << "\n";
    std::cout << "IMPOSSIBLE: " << impossible_count << std::endl;
    std::cout << "UNDERFILL (likely SAT): " << underfill_count << std::endl;
    std::cout << "Answer guess: " << underfill_count << std::endl;
