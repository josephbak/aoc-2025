...
int solve_part2_1(const std::vector<int>& target,
    ...
    for (int j = 0; j < m; j++) {
        x.push_back(ctx.int_const(("x" + std::to_string(j)).c_str()));
        opt.add(x[j] >= 0);  // non-negative presses
    }
    ...
    // Build total presses expression
    ...
    if (opt.check() == z3::sat) {
        ...
        return model.eval(total).get_numeral_int();
    }
...
int main(int argc, char* argv[]) {
    ...
    while (std::getline(file, line)) {
        ...
        total += solve_part2_1(target, buttons);
    }
